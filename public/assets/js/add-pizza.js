...
const handlePizzaSubmit = (event) => {
  ...
  fetch("/api/pizzas", {
    ...
    .catch((err) => {
      console.log(err);
      // when user tries to submit new pizza without an internet connection, the data will be stored in the new_pizza object store in the browser's IndexedDB database called pizza_hunt
      saveRecord(formData);
    });
};
